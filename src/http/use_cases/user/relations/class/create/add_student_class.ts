...
export class addStudentClassUseCase {
    ...
    execute = async (student_id: number, class_id: number) => {
        ...
        // TODO - Uma alternativa mas se for fazer corretamente mudar
        if (student.type.toString() != "Student")
            throw new UserValidationDataError({
                user_type: "Usuario não é um estudante"
            });
        ...
        if (in_class)
            throw new UserValidationDataError({
                user_class: "O aluno já está na Sala"
            });

        await this.user_repository.addStudentClass(user_id_validation.data, class_id_validation.data);
